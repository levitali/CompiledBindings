				...
				XamlObject? ProcessElement(XElement xelement, BindingScope currentBindingScope)
				{
					...
					foreach (var child in xelement.Elements())
					{
						if (child.Name == DataTemplate)
						{
							...
							{
								if (child.Attribute(xDataType) == null && child.Attribute(mxDataType) == null && child.Attribute(DataTypeAttr) == null &&
								...
								ProcessRoot(dataTemplate, child);
						...
						{
							ProcessElement(child, currentBindingScope);
						}
					}
