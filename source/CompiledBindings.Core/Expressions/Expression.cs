...
{
    private readonly List<string> _formatParts;
    private readonly string _lastFormatPart;
    private readonly List<Expression> _expressions;

    public InterpolatedStringExpression(List<string> formatParts, string lastFormatPart, List<Expression> expressions) : base(TypeInfo.GetTypeThrow(typeof(string)))
    {
        _formatParts = formatParts;
        _lastFormatPart = lastFormatPart;
    ...
    protected override Expression CloneReplaceCore(Expression current, Expression replace)
    {
        ...
        {
            expressions.Add(_expressions[i].CloneReplace(current, replace));
        }
        return new InterpolatedStringExpression(_formatParts, _lastFormatPart, expressions);
    }
    ...
    protected override string ToStringCore()
    {
        ...
        sb.Append("$\"");
        for (int i = 0; i < _formatParts.Count; i++)
        {
            sb.Append(_formatParts[i]);
            sb.Append('{');
            sb.Append(_expressions[i]);
            sb.Append('}');
        }
        sb.Append(_lastFormatPart);
        sb.Append("\"");
        return sb.ToString();
    }
}
