...
public class ExpressionTests : IDisposable
{
	...
	public ExpressionTests()
	{
		...
		_targetRoot = new VariableExpression(TypeInfo.GetTypeThrow(typeof(MyControl)), "_targetRoot");
		_dataRoot = new VariableExpression(class1Type, "dataRoot");
	...
	public void Nullables()
	{
		...
		var result = ExpressionParser.Parse(_targetRoot, _dataRoot, expression, stringType, true, ns, out _, out _);
	...
	public void Notifiable()
	{
		...
		var res2 = ExpressionParser.Parse(_targetRoot, _dataRoot, expression2, intType, true, ns, out _, out _);
	...
	public void TestIsExpression()
	{
		...
		result = ExpressionParser.Parse(_targetRoot, _dataRoot, expression, stringType, true, ns, out _, out _);
		Assert.That(result.CSharpCode.Equals(expectedCode));
	...
	{
		var expression = "$SomeResource";
		var expectedCode = "_targetRoot.SomeResource";
		var result = ExpressionParser.Parse(_targetRoot, _dataRoot, expression, intType, true, ns, out _, out _);
		Assert.That(result.CSharpCode.Equals(expectedCode));

		expression = "Method2($SomeResource)";
		expectedCode = "dataRoot.Method2(_targetRoot.SomeResource)";
		result = ExpressionParser.Parse(_targetRoot, _dataRoot, expression, intType, true, ns, out _, out _);
		Assert.That(result.CSharpCode.Equals(expectedCode));
	}
}
